...
class IntersectionEnv(AbstractEnv):
    ...
    def default_config(cls) -> dict:
        ...
        config.update({
            ...
            "normalize_reward": True, #False, TODO
    ...
    def _make_vehicles(self, n_vehicles: int = 10) -> None:
        ...
        """
        # Configure vehicles
        vehicle_type = utils.class_from_path(self.config["other_vehicles_type"]) #IDMVehicle
        ...
        for ego_id in range(0, self.config["controlled_vehicles"]):
            ...
            destination = self.config["destination"] or "o" + str(self.np_random.randint(1, 4)) #o1
            ego_vehicle = self.action_type.vehicle_class(
                             self.road,
                             ego_lane.position(60 + 5*self.np_random.normal(1), 0),
                             speed=ego_lane.speed_limit,
                             heading=ego_lane.heading_at(60)) #MDPVehicle
            try:
            ...
            no_collision_vehicles = []
            no_collision_vehicles.append(ego_vehicle) #ego            
            for v in self.road.vehicles:  # Prevent early collisions
                if v is not ego_vehicle and np.linalg.norm(v.position - ego_vehicle.position) < 20:
                    # self.road.vehicles.remove(v) #changing list we're iterating over
                    pass
                else:
                    no_collision_vehicles.append(v) #others   
            self.road.vehicles = no_collision_vehicles                 
...
class ContinuousIntersectionEnv(IntersectionEnv):
    ...
    def default_config(cls) -> dict:
        ...
        config.update({
            ...
            # "offroad_terminal": True #TODO could help with PPO training?
        })
        return config
...
class IntersectionEnvSimple(IntersectionEnv):
    """ Unified state space """
