...
class ExitEnv(HighwayEnv):
    ...
    def step(self, action) -> Tuple[np.ndarray, float, bool, dict]:
        ...
        obs, reward, terminal, truncate, info = super().step(action) #truncate
        info.update({"is_success": self._is_success()})
        # return obs, reward, terminal, info
        return obs, reward, terminal, truncate, info
...
class SimplifiedExitEnv(ExitEnv):
    """ same as ExitEnv but state space Kinematics + closest 5 vehicles
        and reward based on exiting (intead of exit/pass exit w/o crashing within timelimit)."""
    ...
    def default_config(cls) -> dict:
        ...
        config.update({
            ...
            "collision_reward": -5,
            "high_speed_reward": 0.1,
            "right_lane_reward": 1, #encourage ego vehicle to use right lanes, exploration will encounter exit states easier
            "goal_reward": 10,
            "duration": 9,
            "lanes_count": 4,
        })
    ...
    def _is_success(self):
        ...
        goal_reached = lane_index==("2", "exit", 0)
        return goal_reached
    
    def _reset(self) -> None:
        """make exit closer"""
        self._create_road(exit_position=200)
        self._create_vehicles()    
