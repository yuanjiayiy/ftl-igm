...
n_demos = 100
vehicles_count = 2
for scenario_text,version in [("highway",1)]:
    print(scenario_text)
    env_name = f"{scenario_text.replace('_','-')}-v{version}"
    video_folder = f"{save_dir}/{scenario_text}_{version}_{vehicles_count}/videos"
    demos_pkl = f"{save_dir}/{scenario_text}_{version}_{vehicles_count}/demos.pkl"
