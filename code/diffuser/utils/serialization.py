...
def load_diffusion(*loadpath, epoch='latest', device='cuda:0', seed=None, load_dataset=True):
    ...
    if load_dataset:
        ...
        trainer.load(epoch)   
        ema = trainer.ema_model
    else:
        dataset = None
        trainer = None
        ema = None

    return DiffusionExperiment(dataset, renderer, model, diffusion, ema, trainer, epoch)
